...
async function isAllowedDomain(tabId: number): Promise<boolean> {
  try {
    const tab = await chrome.tabs.get(tabId);
    if (!tab.url) return false;
    return tab.url.includes('sb-idotrow.bemcorp.net');
  } catch (error) {
...
export async function getScrollInfo(tabId: number): Promise<[number, number]> {
  ...
  if (!(await isAllowedDomain(tabId))) {
    logger.error('Tab is not on an allowed domain, cannot get scroll info');
    throw new Error('Tab is not on an allowed domain');
  }
...
export async function getMarkdownContent(tabId: number, selector?: string): Promise<string> {
  // Check if the tab is on an allowed domain
  if (!(await isAllowedDomain(tabId))) {
    logger.error('Tab is not on an allowed domain, cannot get markdown content');
    throw new Error('Tab is not on an allowed domain');
  }
...
export async function getReadabilityContent(tabId: number): Promise<ReadabilityResult> {
  // Check if the tab is on an allowed domain
  if (!(await isAllowedDomain(tabId))) {
    logger.error('Tab is not on an allowed domain, cannot get readability content');
    throw new Error('Tab is not on an allowed domain');
  }
...
export async function getClickableElements(
  ...
  try {
    // Check if the tab is on an allowed domain
    if (!(await isAllowedDomain(tabId))) {
      logger.error('Tab is not on an allowed domain, cannot get clickable elements');
      throw new Error('Tab is not on an allowed domain');
    }
...
async function _buildDomTree(
  ...
  // Domain check is done in the calling function
...
export async function removeHighlights(tabId: number): Promise<void> {
  try {
    // Check if the tab is on an allowed domain
    if (!(await isAllowedDomain(tabId))) {
      logger.error('Tab is not on an allowed domain, cannot remove highlights');
      return;
    }
