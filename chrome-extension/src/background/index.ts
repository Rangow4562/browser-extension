...
async function isScriptInjected(tabId: number): Promise<boolean> {
  try {
    // First check if the tab is on an allowed domain
    const tab = await chrome.tabs.get(tabId);
    if (!isAllowedDomain(tab.url)) {
      console.log('Tab is not on an allowed domain, skipping script injection check');
      return false;
    }
...
async function injectBuildDomTree(tabId: number) {
  try {
    ...
    const tab = await chrome.tabs.get(tabId);
    if (!isAllowedDomain(tab.url)) {
      console.log('Tab is not on an allowed domain, skipping script injection');
      return;
    }
...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  if (tabId && changeInfo.status === 'complete' && tab.url) {
    // Only inject scripts for the ather domain
    if (isAllowedDomain(tab.url)) {
      console.log('ather domain detected, injecting scripts...');
      await injectBuildDomTree(tabId);
    } else {
      console.log('Non-allowed domain detected, skipping script injection');
    }
  }
});
...
async function setupExecutor(taskId: string, task: string, browserContext: BrowserContext) {
  // Get providers and agent models with default configurations
  ...
  const plannerLLM = createChatModel(providers[plannerModel.provider], plannerModel);

  const validatorModel = agentModels[AgentNameEnum.Validator];
  const validatorLLM = createChatModel(providers[validatorModel.provider], validatorModel);
