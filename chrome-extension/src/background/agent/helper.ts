...
function createAzureOpenAIChatModel(providerConfig: ProviderConfig, modelConfig: ModelConfig): BaseChatModel {
  // For Azure OpenAI, the deployment name is used instead of the model name
  // If no deployment name is provided, use the model name as a fallback
  const deploymentName = providerConfig.azureOpenAIApiDeploymentName || modelConfig.modelName;

  // Log the configuration for debugging
  console.log('Azure OpenAI Configuration:', {
    apiKey: providerConfig.apiKey ? '***' : 'not set',
    instanceName: providerConfig.azureOpenAIApiInstanceName,
    deploymentName,
    apiVersion: providerConfig.azureOpenAIApiVersion,
  });
  ...
  const args: {
    ...
    azureOpenAIApiInstanceName: providerConfig.azureOpenAIApiInstanceName,
    azureOpenAIApiDeploymentName: deploymentName,
    azureOpenAIApiVersion: providerConfig.azureOpenAIApiVersion || '2025-01-01-preview',
  };
...
export function createChatModel(providerConfig: ProviderConfig, modelConfig: ModelConfig): BaseChatModel {
  ...
  switch (modelConfig.provider) {
    ...
    case ProviderTypeEnum.AzureOpenAI: {
      return createAzureOpenAIChatModel(providerConfig, modelConfig);
    }
