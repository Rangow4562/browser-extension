...
// Function to check if a URL is allowed (only sb-idotrow.bemcorp.net domain)
function isAllowedDomain(url: string | undefined): boolean {
  if (!url) return false;
  return url.includes('sb-idotrow.bemcorp.net');
}
export default class BrowserContext {
  ...
  public async getCurrentPage(): Promise<Page> {
    ...
    if (!this._currentTabId) {
      ...
      if (!tab?.id) {
        ...
        // Check if the active tab is on an allowed domain
        if (!isAllowedDomain(tab.url)) {
          // If not, open a new tab with the allowed domain
          logger.info('Active tab is not on allowed domain, opening new tab with allowed domain');
          const newTab = await chrome.tabs.create({ url: this._config.homePageUrl });
          if (!newTab.id) {
            throw new Error('No tab ID available');
          }
    ...
    if (!existingPage) {
      ...
      // Check if the tab is on an allowed domain
      if (!isAllowedDomain(tab.url)) {
        // If not, open a new tab with the allowed domain
        logger.info('Current tab is not on allowed domain, opening new tab with allowed domain');
        const newTab = await chrome.tabs.create({ url: this._config.homePageUrl });
        if (!newTab.id) {
          throw new Error('No tab ID available');
        }
  ...
  public async switchTab(tabId: number): Promise<Page> {
    ...
    if (!isAllowedDomain(tab.url)) {
      logger.error('Cannot switch to tab with non-allowed domain:', tab.url);
      throw new Error('Cannot switch to tab with non-allowed domain');
    }
  ...
  public async navigateTo(url: string): Promise<void> {
    ...
    if (!isAllowedDomain(url)) {
      logger.error('Cannot navigate to non-allowed domain:', url);
      throw new Error('Cannot navigate to non-allowed domain');
    }
  ...
  public async openTab(url: string): Promise<Page> {
    // Check if the URL is allowed
    if (!isAllowedDomain(url)) {
      logger.error('Cannot open tab with non-allowed domain:', url);
      throw new Error('Cannot open tab with non-allowed domain');
    }
