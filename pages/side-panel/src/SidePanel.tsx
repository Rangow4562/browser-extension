...
// No longer need FaBuilding icon
...
const SidePanel = () => {
  ...
  return (
    <div>
      <div
        className={`flex h-screen flex-col ${isDarkMode ? 'bg-bem-dark' : 'bg-gradient-to-br from-white to-bem-light'} overflow-hidden border ${isDarkMode ? 'border-bem-primary/30' : 'border-bem-primary/20'} rounded-2xl shadow-bem-lg animate-fadeIn`}>
          ...
          <div className="header-logo">
            {showHistory ? (
              <button
                ...
                className={`${isDarkMode ? 'text-bem-accent hover:text-white' : 'text-bem-secondary hover:text-bem-primary'} cursor-pointer font-medium flex items-center gap-1 transition-colors`}
                aria-label="Back to chat">
                <span className="text-lg">←</span> Back
              </button>
            ) : (
              <div className="flex items-center gap-2">
                <img src="/icon-128.png" alt="ather Logo" className="size-8" />
                <span className="font-semibold rgba(255, 255, 255, 0.8)">ather</span>
              </div>
            )}
          </div>
        ...
        {showHistory ? (
          <div className="flex-1 overflow-hidden animate-slideUp p-4">
            <ChatHistoryList
          ...
          <>
            {messages.length === 0 && (
              <>
                <div
                  className={`border-t ${isDarkMode ? 'border-bem-primary/20' : 'border-bem-primary/10'} mb-4 p-3 shadow-bem backdrop-blur-sm -webkit-backdrop-filter: blur(8px)`}>
                  <ChatInput
            ...
            <div
              className={`scrollbar-gutter-stable flex-1 overflow-x-hidden overflow-y-auto scroll-smooth p-4 ${isDarkMode ? 'bg-bem-dark/90' : 'bg-white/50'} animate-slideUp`}>
              <MessageList messages={messages} isDarkMode={isDarkMode} />
              <div ref={messagesEndRef} />
            </div>
            {messages.length > 0 && (
              <div
                className={`border-t ${isDarkMode ? 'border-bem-primary/20' : 'border-bem-primary/10'} p-3 shadow-bem backdrop-blur-sm -webkit-backdrop-filter: blur(8px)`}>
                <ChatInput
                  onSendMessage={handleSendMessage}
                  onStopTask={handleStopTask}
                  disabled={!inputEnabled || isHistoricalSession}
