...
import { AgentNameEnum, ProviderTypeEnum } from './types';
...
// Helper function to get model parameters for an agent and provider
...
const defaultAgentModels: Record<AgentNameEnum, ModelConfig> = {
  [AgentNameEnum.Planner]: {
    ...defaultModelConfig,
    parameters: getModelParameters(AgentNameEnum.Planner, ProviderTypeEnum.AzureOpenAI),
  },
  [AgentNameEnum.Navigator]: {
    ...defaultModelConfig,
    parameters: getModelParameters(AgentNameEnum.Navigator, ProviderTypeEnum.AzureOpenAI),
  },
  [AgentNameEnum.Validator]: {
    ...defaultModelConfig,
    parameters: getModelParameters(AgentNameEnum.Validator, ProviderTypeEnum.AzureOpenAI),
  },
};

const storage = createStorage<AgentModelRecord>(
  'agent-models',
  { agents: defaultAgentModels },
  {
...
export const agentModelStore: AgentModelStorage = {
  ...
  getAllAgentModels: async () => {
    ...
    // If no agents are configured, use the default configurations
    if (Object.keys(agents).length === 0) {
      agents = defaultAgentModels;
      // Save the default models to storage
